...
已知训练数据集$T = \left \{ \left (x_1 ,y_1 \right ) ,\dots ,\left ( x_n,y_n \right )  \right\} $, 损失函数$l\left ( y_i,\hat{y_i}  \right ) $，正则化项$\Omega \left ( f_k \right ) $。  
...
$f\left ( y_i,x \right ) \approx f\left (y_i, x_0 \right ) +f'\left (y_i,  x_0\right )  \left ( x-x_0 \right )+\frac{f''\left ( y_i, x_0 \right ) }{2} \left ( x-x_0 \right )^2$  
类似地，对$l\left ( y_i,x \right )$在$\hat{y_i}^\left (t-1 \right )$处进行二阶泰勒展开得： 
$f\left ( y_i,x \right ) \approx f\left (y_i, \hat{y_i}^\left (t-1 \right ) \right ) +f'\left (y_i,  \hat{y_i}^\left (t-1 \right )\right )  \left ( x-\hat{y_i}^\left (t-1 \right ) \right )+\frac{f''\left ( y_i, \hat{y_i}^\left (t-1 \right ) \right ) }{2} \left ( x-\hat{y_i}^\left (t-1 \right )\right )^2 $  
