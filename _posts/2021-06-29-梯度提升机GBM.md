...
## 2.3 公式推导  
假设有训练样本{ $x_i,y_i$ }$, i=1,...,n, 在第k-1(k<n)轮获得的累积模型为$F_{m-1}\left ( x \right )$, 则第k轮的弱学习器$h\left ( x \right ) $可以通过下列公式得到：  
...
即：$\aligned{center}\omega x+b = \log \frac{p}{1-p}\begin{aligned}$  

且损失函数是Cross Entropy Loss交叉熵损失，即：  
$\begin{aligned}Loss = -y\log p - \left ( 1-y \right ) \log \left (1-p  \right )\end{aligned}$
