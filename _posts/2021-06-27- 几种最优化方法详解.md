...
并不是所有的方程都有求根公式，或者求根公式很复杂，导致求解困难。利用牛顿法，可以**迭代求解**。原理是利用泰勒公式<a target="_blank" href="https://baike.baidu.com/item/%E6%B3%B0%E5%8B%92%E5%85%AC%E5%BC%8F">[3]</a>, 在$x_{0}$处展开到一阶，即$f\left ( x \right )=f\left (  x_{0} \right ) + f'\left (  x_{0} \right )\left ( x-x_{0} \right ) $，然后求解
$f\left ( x \right )=0$，解得$x = x_{1}=x_{0} - \frac{f\left ( x_{0} \right )}{f'\left ( x_{0} \right )} $。  
...
设，${f\left ( x\right )}$具有二阶连续偏导数，若第k次的迭代值为$x^{\left (k  \right )}$， 则可将${f\left ( x\right )}$在$x^{\left (k  \right )}$附近进行泰勒二阶展开：  
...
其中$g_{k}=g\left ( x^{\left ( k \right ) } \right ) =\bigtriangledown f\left ( x^{\left ( k \right ) } \right ) $是${f\left ( x\right )}$的梯度向量在$ x^{\left ( k \right ) } $的值，而$ H\left ( x^{\left ( k \right ) } \right )$是${f\left ( x\right )}$的Hessian海塞矩阵<a target="_blank" href="https://baike.baidu.com/item/%E9%BB%91%E5%A1%9E%E7%9F%A9%E9%98%B5">[4]</a>  
    
$ H\left ( x \right ) = \left [ \frac{\mathrm{d^{2}} f}{\mathrm{d} x_{i}\mathrm{d} x_{j}}  \right ]_{n\times n}   
$在$ x^{\left ( k \right ) } $的值。  
而函数${f\left ( x\right )}$有极值的必要条件是在极值点处一阶导数为0（即梯度向量为0），且当$H\left ( x^{\left ( k \right ) } \right )$是**正定矩阵**（对任何非零向量z，都有$z^{T}Mz>0$）时, 取到极小值。  
